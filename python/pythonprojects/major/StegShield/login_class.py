...
import hashlib, re
from stegShield import StegoTool
import os
...
class Login:
    def __init__(self,root):
        ...
        Label(root,image=self.root.img, bg='white').place(x=50,y=50)
        self.create_widgets()
    ...
    # ---------------------------------------
    def on_Penter(self,e):
        if self.passwd.get() == "Password":
            self.passwd.delete(0,'end')
    def on_Pleave(self,e):
        if not self.passwd.get():
            self.passwd.insert(0,'Password')
    # ---------------------------------------
    # def check_focus(self, e):  # type: ignore
    #     # Check focus for email
    #     if not self.mail.get() and self.mail != self.root.focus_get():
    #         self.on_Mleave(e)
    #     # Check focus for password
    #     if not self.passwd.get() and self.passwd != self.root.focus_get():
    #         self.on_Pleave(e)
    ...
    def create_widgets(self):
        ...
        Frame(self.frame,width=295,height=2,bg='black').place(x=25,y=177)
         ...
         # Bind the root window to check for focus changes
        # self.root.bind('<FocusOut>', self.check_focus)
        ...
        self.sign_up=Button(self.frame,width=6,text='Sign up',bg='white',fg='#57a1f8',border=0,cursor='hand2',command=self.open_signup_page).place(x=215,y=270)

    def open_signup_page(self):
        signup_window = Toplevel(self.root)
        signup_page = Signup(signup_window)
    ...
    #     main_page = StegoTool(main_window)

    def signin(self):
        user_email = self.mail.get()
        user_password = self.passwd.get()
                ...
                if connection.is_connected():
                    ...
                    if row:
                        ...
                        if hashed_password == stored_password:
                            ...
                            # os.system("python stegShield.py")
