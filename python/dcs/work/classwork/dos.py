...
def main():
    while True:
        ...
        list_attacks = ["GoldenEye", "SlowHTTPTest", "THC-SSL-DOS", "Slowloris", "go back"]
        for i in range(len(list_attacks)):
            print(colors.options,f"{i}) {list_attacks[i]}".title(),colors.reset)
        option = input(f"\n {colors.select}Select An Option ->{colors.reset}  ")
        if option == "0":
            print("\n[+] GoldenEye")
            github = github_getting_text("https://www.kali.org/tools/goldeneye/", 'p', 0)
            template.template("goldeneye","goldeneye -h",github.strip(),{
            ...
            print("\n[+] Slowhttptest")
            github = github_getting_text("https://www.kali.org/tools/slowhttptest/", 'p', 1)
            template.template("slowhttptest","slowhttptest -h",github.strip(),{
            ...
            print("\n[+] THC-SSL-DOS")
            github = github_getting_text("https://www.kali.org/tools/thc-ssl-dos/", 'p', 1)
            github += github_getting_text("https://www.kali.org/tools/thc-ssl-dos/", 'p', 2)
            github += github_getting_text("https://www.kali.org/tools/thc-ssl-dos/", 'p', 3)
            template.template("thc-ssl-dos","thc-ssl-dos -h",github.strip(),{
            ...
            template.template("Slowloris","python slowloris -h",github.strip(),{
                ...
                }, link="https://github.com/gkbrk/slowloris.git", method="github", github_install='git clone https://github.com/gkbrk/slowloris.git', github_check="slowloris")
